...
> 舊版 D3「回應／決志卡 +200 KP」橋段**已取消**：Day3 沿用 Day2 全套，不加開強化天國攤、無回應卡、無奉獻 bonus。`response_card` 欄位/endpoint 僅保留相容，不使用。
...
1. 先掃該生逾時 pending（≥15 分）→ `expired` 作廢、**扣 100 罰款**。
...
**抽取池（9 款，加權隨機，不重複；終極密碼/記憶翻牌/七巧板權重 1、其餘 2）**

| 難度 | 固定獎勵 | 款數 | 遊戲 |
|---|---|---|---|
| 低 | 100 | 2 | 終極密碼、搬家人工 |
| 中 | 200 | 5 | 疊杯子、丟紙飛機、拍氣球、比手畫腳、記憶翻牌 |
| 高 | 300 | 2 | 顏色分類、七巧板 |

> 單一任務期望獎勵 200（免手續費即單抽期望淨值，SOT §3）。逾時作廢扣 100 罰款（免手續費後唯一嚇阻）；N 個任務共用同一個 15 分鐘倒數，抽越多、時限內跑完全部關卡越難，這是新的剎車機制（時間風險取代舊版手續費沉沒成本）。
...
- uid 取自實體刷卡。建表前可用「綁卡」流程：感應 → 輸入姓名 → 依才幹規則選起始金（5000/2000/1000）→ POST。
...
```jsonc
{
  "uid":"04A1B2C3", "name":"王小明", "seed":5000,
